...
public abstract class BaseGuavaCache<K, V> {
    ...
    /**
     * 用于初始化缓存值（某些场景下使用，例如系统启动检测时，缓存加载是否正常）
     */
    public abstract Map<? extends K, ? extends V> loadValueWhenStarted();
     ...
     */
    protected abstract V reloadValueWhenExpired(K key) throws Exception;
     ...
     * @return
     */
    protected BaseGuavaCache<K, V> put(K key, V value) {
        try {
            getCache().put(key, value);
            return this;
        } catch (Exception e) {
            logger.error("put 发生异常，key{} ", key, e);
            throw e;
        }
    }
     ...
     * @return
     */
    protected BaseGuavaCache<K, V> putAll(Map<? extends K, ? extends V> mapInput) {
        try {
            getCache().putAll(mapInput);
            return this;
        } catch (Exception e) {
            logger.error("putAll 发生异常， ", e);
            throw e;
        }
    }
    ...
    private void initCache() {
        Map<? extends K, ? extends V> mapInit = loadValueWhenStarted();
        if (mapInit != null && mapInit.size() > 0) {
            putAll(mapInit);
        }
    }
    ...
    private LoadingCache<K, V> getCache() {
        if (cache == null) {
            synchronized (this) {
                if (cache == null) {
                    //创建CacheBuilder 并设置maxSize
                    CacheBuilder<Object, Object> cacheBuilder = CacheBuilder.newBuilder()
                            .maximumSize(maxSize);
                    //设置过期策略(如果refreshDuration被使用者设过则使用该策略)
                    if (refreshDuration != 10 && refreshDuration > 0) {
                    ...
                    cache = cacheBuilder.build(new CacheLoader<K, V>() {
                        ...
                        public V load(K key) throws Exception {
                            return reloadValueWhenExpired(key);
                        }
