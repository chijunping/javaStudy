...
public class Dom4j_CRUD {
	...
	public void testDom4j() throws Exception {
		...
		Document document = reader.read(new File("src/main/resources/TestFile/testDom4j.xml"));
		// 获取根节点元素对象
		Element node = document.getRootElement();
		// 去除空格和不去除空格方法对比
		System.out.println(">>>" + node.getTextTrim());
		System.out.println(">>>" + node.getText());
		System.out.println("==========去除空格和不去除空格方法对比=============");
		// 遍历所有的元素节点
		listNodes(node);
	...
	public void writer(Document document) throws Exception {
		...
		XMLWriter writer = new XMLWriter(new OutputStreamWriter(new FileOutputStream(new File("src/main/resources/TestFile/02.xml")), "UTF-8"), format);
		// 写入
